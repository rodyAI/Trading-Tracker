import type { MarketCandle, MarketDataProviderId, MarketQuote } from "../../../shared/src/types.js";
...
export class MarketDataService {
  ...
  async getDailyCandles(symbolInput: string): Promise<MarketCandle[]> {
    ...
    return this.provider === "alphavantage"
      ? this.getAlphaVantageDailyCandles(symbol)
      : this.getYahooDailyCandles(symbol);
  }
  ...
  private async getAlphaVantageDailyCandles(symbol: string): Promise<MarketCandle[]> {
    ...
    if (candles.length === 0) {
      throw new Error(`No daily candle history returned for ${symbol}.`);
    }
